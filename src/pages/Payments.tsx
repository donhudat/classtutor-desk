...
function AttendanceBreakdown({
  ...
  const q = useQuery({
    queryKey: ["payment-breakdown", studentId, classId, month],
    ...
    queryFn: async () => {
      ...
      if (ids.length) {
        const { data, error: e2 } = await supabase
          .from("attendances")
          .select("session_id, status, note, checked_in_at")
            ...
            <ul className="space-y-1">
              {q.data!.map((s: any) => {
                ...
                const tone = isCancelled
                  ? SESSION_TONE.cancelled
                  : att
                  ? ATT_TONE[att] ?? SESSION_TONE[s.status]
                  : SESSION_TONE[s.status] ?? SESSION_TONE.scheduled;
                ...
                return (
                  <li
                    key={s.id}
                    className={`flex items-center justify-between gap-2 rounded border px-2 py-1.5 text-xs ${tone}`}
                  >
                      ...
                      <TableCell className="text-center">
                        {r.class_enrollments?.classes?.id ? (
                          <AttendanceBreakdown
                            ...
                            classId={r.class_enrollments.classes.id}
                            month={r.month}
                          >
                            <span className="inline-flex flex-col items-center underline-offset-2 hover:underline">
                              <span>
                                <span className="font-medium">{r.attended_count + r.late_count}</span>
                                <span className="text-muted-foreground"> / {r.session_count}</span>
                              </span>
                              {(r.late_count > 0 || r.absent_count > 0 || r.excused_count > 0) && (
                                <span className="text-[11px] text-muted-foreground">
                                  {r.late_count > 0 && `muộn ${r.late_count}`}
                                  {r.late_count > 0 && (r.absent_count > 0 || r.excused_count > 0) && " · "}
                                  {r.absent_count > 0 && `vắng ${r.absent_count}`}
                                  {r.absent_count > 0 && r.excused_count > 0 && " · "}
                                  {r.excused_count > 0 && `phép ${r.excused_count}`}
                                </span>
                              )}
                            </span>
                          </AttendanceBreakdown>
